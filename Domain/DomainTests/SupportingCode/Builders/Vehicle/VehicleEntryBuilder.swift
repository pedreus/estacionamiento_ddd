...
public class VehicleEntryBuilder {
    ...
    func build() throws -> Entry {
        let unwrappedVehicle: Vehicle?
        if let vehicle = vehicle {
            unwrappedVehicle = vehicle
        } else {
            unwrappedVehicle = try VehicleBuilder().build()
        }
        
        return try Entry(entryDateTime: entryDate, vehicle: unwrappedVehicle!)
    }
}
